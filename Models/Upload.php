<?php
	...
	class Upload{
		...
		const IMG_STORAGE_PATH = "/Storage/images/";
		const IMG_PATH = ROOT . "Storage/images/";
		const TARGET_WITH = 320;
				...
				if ((is_array($this->_upload)) && (is_array($this->_allowed))) {
					...
					if (in_array($extension, $this->_allowed)) {
						...
						if ($this->_upload['size'] < $this->_size) {
							...
							$newTemp = $this->resize($tmpname, $this->_dir.$filename, $filename);
						}else{
		...
		private function resize($file, $dirName, $fileName) {

			$source_properties = getimagesize($file);
			$image_type = $source_properties[2]; 
			if( $image_type == IMAGETYPE_JPEG ) {   
				$image_resource_id = imagecreatefromjpeg($file);  
				$target_layer = $this->fn_resize($image_resource_id,$source_properties[0],$source_properties[1], $file);
				
				if (imagejpeg($target_layer,$dirName)) {
			...
			elseif( $image_type == IMAGETYPE_GIF )  {  
				...
				$target_layer = $this->fn_resize($image_resource_id,$source_properties[0],$source_properties[1], $file);
				if (imagegif($target_layer,$dirName)) {
					$this->_result['type'] = "success";
					$this->_result['message'] = "File Has been uploaded";
					$this->_result['path'] = $fileName;
				}else{
			...
			elseif( $image_type == IMAGETYPE_PNG ) {
				...
				$target_layer = $this->fn_resize($image_resource_id,$source_properties[0],$source_properties[1], $file);
				if (imagepng($target_layer,$dirName)) {
					$this->_result['type'] = "success";
					$this->_result['message'] = "File Has been uploaded";
					$this->_result['path'] = $fileName;
				}else{
		...
		private function fn_resize($image_resource_id,$width,$height, $originalFile) {
			$target_width = $this::TARGET_WITH;
